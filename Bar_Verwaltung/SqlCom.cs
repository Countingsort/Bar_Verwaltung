...
namespace Bar_Verwaltung
{
    internal class SqlCom
    {
        ...
        {
            string pri = price.ToString(new CultureInfo("en-US"));
            cmd.CommandText = string.Format("Update TStock Set Drink = {0},Item='{1}',Quantaty={2},Price={3},Ingrediens='{4}',Percentage={5},fsk='{6}' where ID={7};", drink, name, quantaty, pri, ingrediens, percentage, fsk,ID);
            con.Open();
            cmd.ExecuteNonQuery();
            con.Close();
        }
